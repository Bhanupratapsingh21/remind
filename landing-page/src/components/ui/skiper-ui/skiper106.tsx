...
const SmoothInput = ({
  ...
  const activeFontSize = propFontSize || 26;
  ...
  // Underline minimalist variant (matching Skiper106 demo screenshots)
  if (variant === "underline") {
    return (
      <div className={cn("w-full max-w-[560px] mx-auto", wrapperClassName)}>
        {label && (
          <label className="block text-[10px] sm:text-xs font-mono uppercase tracking-widest text-neutral-400 mb-2 select-none">
            {label}
          </label>
        )}

        <div className="relative flex items-center border-b border-neutral-200/90 focus-within:border-[#151515] pb-2 transition-colors">
          <div
            ref={containerRef}
            className="relative grid grid-cols-1 p-0 flex-1 min-w-0"
            style={{ caretColor: "transparent", fontSize: activeFontSize }}
          >
            <input
              {...props}
              ref={inputRef}
              ...
              className={cn(
                "w-full bg-transparent outline-none placeholder:text-neutral-300 text-[#151515] font-normal tracking-tight col-start-1 col-end-2 row-start-1 row-end-2 text-inherit pr-2",
                className,
              )}
              style={style}
              value={inputValue}
            ...
            />
            <motion.div
              className="bg-[#151515] pointer-events-none col-start-1 col-end-2 row-start-1 row-end-2 h-[0.9em] w-[2px] self-center"
              style={{ x: springCaretX, opacity: caretOpacity }}
            />
          </div>

          <div className="shrink-0 ml-3 flex items-center select-none">
        ...
        {statusMessage && <div className="mt-2.5">{statusMessage}</div>}
      </div>
    );
  }
